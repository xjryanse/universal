...
trait PageAuthSetTrait {
    ...
    public static function paginateForAuthSet(){
        ...
        $pageId                 = Arrays::value($param, 'page_id');
        $res['data']            = self::listForAuthSet($pageId);
        $res['fdynFields']      = self::fDynFieldsForAuthSet($pageId);
        $res['per_page']        = 9999;
     ...
     * 20240413：用于动态设置权限
     */
    protected static function listForAuthSet($pageId){
     ...
     * @return string
     */
    protected static function fDynFieldsForAuthSet($pageId) {
        $titleKey = self::$titleKey;
        $con[]  = ['page_id', '=', $pageId];
        $con[]  = ['status', '=', 1];
        $con[]  = [$titleKey,'<>',''];        
